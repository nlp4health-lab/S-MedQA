...
predict_indices = set(range(len(df_gold))) - set(calibration_indices)
...
with open(output_jsonl_file, "a") as f:
    # Dictionary to store non-conformity scores
    for idx in predict_indices:
        ...
        # Get all labels <= threshold
        predicted_labels = []
        for label, count in response_frequencies.items():
            if 1-(count / total_responses) <= conformity_threshold:
