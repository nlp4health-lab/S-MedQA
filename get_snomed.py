...
endpoint_builder.set_api_endpoint("https://snowstorm.mi-x.nl")
...
def map_finding_via_hpo(original_term, disease_to_specialty, endpoint_builder):
    ...
    # Load HPO data
    try:
        with open("/home/xyan1/srp_project/Token/hp.json", 'r') as file:
    ...
    while True:
        ...
        if matching_labels:
            ...
            if specialties:
                # print(f"Specialties found for term '{current_term}': {specialties}")
                return specialties
...
def simplify_term_using_nltk(term):
    ...
    simplified_tokens = []  # Initialize the list to store simplified tokens
    adjective_found = True  # Flag to track whether we're still encountering adjectives
...
# term_to_test = input("Enter a term to map: ")
# result = map_term_to_specialty(term=term_to_test, disease_to_specialty=disease_to_specialty, endpoint_builder=endpoint_builder)
# # result = simplify_term_using_nltk(term_to_test)
# print(result)
...
def process_multiple_datasets_with_cache(dataset_paths, disease_to_specialty, endpoint_builder, output_file):
    ...
    """
    all_results = {}  # List to store results from all datasets
    cache = {}  # Cache to store already mapped terms
    ...
    for dataset_path in dataset_paths:
        try:
            ...
            # # Map each term in the dataset to specialties
            # for term in terms:
            #     if term in cache:
            #         # Skip mapping if the term is already in the cache
            #         print(f"Term '{term}' already mapped. Using cached result.")
            #         all_results[term] = cache[term]
            #     else:
            #         print(f"Mapping term '{term}' to specialties...")
            #         term_specialty_mapping = map_term_to_specialty(term, disease_to_specialty, endpoint_builder)
            #         cache[term] = term_specialty_mapping[term]  # Save the mapping to the cache
            #         all_results[term] = term_specialty_mapping[term]
...
# Example usage
dataset_paths = [
    # "/home/xyan1/LLaMA-Factory/data/Cardiology_idx_test.json"
    # "/home/xyan1/srp_project/Token/specialty_mapping/medqa_six_idx_train.json"
    "/home/xyan1/LLaMA-Factory/data/medqa_six_idx_train.json"
    # "dataset2.json", 
    # "dataset3.json", 
    # "dataset4.json", 
    # "dataset5.json", 
    # "dataset6.json"
]  # Replace with the actual paths to your 6 JSON files

output_file = "/home/xyan1/srp_project/term_specialty_mapping_train.json"  # The file where results will be saved
